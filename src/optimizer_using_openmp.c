...
double try_update_triplets(int iters, int id, int gift_to_try, int *tch_ret,
                           int *tsh_ret, double *score_ret) {
  int i, z, c, j;
  int tr_id1 = id;
  int tr_id2 = id + 1;
  int tr_id3 = id + 2;
...
#pragma omp parallel for private(j) schedule(dynamic, 1) num_threads(numthreads)
  for (z = 0; z < iters; z++) {
    ...
    new_score = (tch_new - 10000000.0) * (tch_new - 10000000.0) *
                (tch_new - 10000000.0);
    new_score +=
        (tsh_new - 1000000.0) * (tsh_new - 1000000.0) * (tsh_new - 1000000.0);
...
double try_update_twins(int iters, int id, int gift_to_try, int *tch_ret,
                        int *tsh_ret, double *score_ret) {
  int i, z, c, j;
  int tr_id1 = id;
  int tr_id2 = id + 1;
...
double try_update_singles(int iters, int id, int gift_to_try, int *tch_ret,
                          int *tsh_ret, double *score_ret) {
  int i, z, c, j;
  int tr_id1 = id;
  int r1;
    ...
    if (1) {
      ...
      for (i = 0; i < 5001; i += 3) {
        ...
        for (j = 0; j < 1000; j++) {
          if (preds[i] == j)
            continue;
          cur_score = try_update_triplets(10 * ITERS_PER_UPDATE, i, j, &tch,
                                          &tsh, &cur_score);
        }
      }
    ...
    if (1) {
      ...
      for (i = 5001; i < 45001; i += 2) {
        ...
        for (j = 0; j < 1000; j++) {
          if (preds[i] == j)
            continue;
          cur_score = try_update_twins(5 * ITERS_PER_UPDATE, i, j, &tch, &tsh,
                                       &cur_score);
        }
      }
    ...
    if (1) {
      ...
      for (i = 45001; i < 1000000; i++) {
        ...
        for (j = 0; j < 1000; j++) {
          ...
          cur_score = try_update_singles(ITERS_PER_UPDATE, i, j, &tch, &tsh,
                                         &cur_score);
        }
      }
